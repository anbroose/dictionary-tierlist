    ...
    <div className={styles.fullScreen}>
      <div className={styles.content}>
          ...
          <tbody>
            {["S", "A", "B", "C", "D", "E", "F"].map((tier) => (
              <tr key={tier} className={styles[tier.toLowerCase()]}>
                ...
                <td className={`${styles.tierWords} ${isZoomedOut ? styles.zoomedOut : ""}`}>
                  {groupedByTier[tier]?.map((word) => (
                    <div key={word} className={styles.rankItem}>{word}</div>
                  ))}
                </td>
              </tr>
            ))}
        ...
        <div className={styles.buttonContainer}>
          {!isZoomedOut && (
            <button className={styles.zoomButton} onClick={handleZoomOut}>Fit All Words</button>
          )}
          <button className={styles.closeButton} onClick={() => window.location.reload()}>Start Over</button>
        </div>
      </div>
    </div>
  );
